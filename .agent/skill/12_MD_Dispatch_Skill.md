...
description: "선택된 파일 형식에 따라 /md-html, /md-ppt-text, /md-xlsx 또는 /md-pdf를 자동 호출하는 변환 디스패처"
category: "skill"
tags:
  - md
  - dispatch
...
- PDF 문서 계열이면 OCR 텍스트 정제 및 헤더/푸터 노이즈 필터링을 위해 `/md-pdf`를 호출한다.
...
- `@include skill/15_MD_PDF_Convert_Skill.md`
...
1. 대상 파일 또는 폴더를 확인한다.
2. 확장자와 소스 유형을 판별한다.
3. HTML 계열이면 `/md-html`을, PPT 계열이면 `/md-ppt-text`를, Excel 계열이면 `/md-xlsx`를, PDF 계열이면 `/md-pdf`를 자동 호출한다.
