...
> **Version 1.2 업데이트**: 
> 1. **하이브리드 워크플로우 구축**: 파이썬 파서(Parser) 모듈과 텍스트 정제 서브에이전트(Subagent) 아키텍처 분리 및 통합. (외부 API 없이 Antigravity가 직접 정제 단계를 자율 수행)
...
> 4. **PPT 파싱 무결성 강화 및 예외 처리**: 복잡한 유니코드 환경에서의 대규모 치환(UTF-8 Stream Error) 튕김 현상을 우회하는 Python 스크립트 강제 주입(Injection) 지침 하네스 추가.
> 5. **슬라이드 구조 정밀 렌더링 규정 고도화**: 거버넌스 문장 위치 기반 판별 기준(제목 직하단 텍스트) 및 슬라이드 제목 포맷 `# [번호] Page - [제목]` 규칙 의무화.
