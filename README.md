...
- `/md-pdf` PDF 변환 스킬을 신설하여, 반복되는 헤더/푸터 노이즈를 식별/제거하고 파편화된 다이어그램 OCR 텍스트를 논리적으로 선형화하도록 파이프라인을 구축했습니다.
- PPT 렌더링 스킬(`/md-ppt-render`) 실행 시 파편화된 데이터 방치를 원천 차단하는 강력한 훅(`03_PPT_Parsing_Hook.md`)을 추가하고, 무분별한 도식화 대신 텍스트 서술 최우선 원칙과 Vision(이미지 대조) 기반 문장/단어 복원 의무를 `08_PPT_Parsing_Rule.md` 규칙에 명시했습니다.
...
- `/md` : 선택된 파일 형식에 따라 `/md-html`, `/md-ppt-text`, `/md-xlsx` 또는 `/md-pdf`를 자동 호출하는 변환 디스패처
...
- `/md-pdf` : PDF 문서 OCR 텍스트 정밀 추출 및 불필요한 메타데이터(헤더/푸터) 필터링
