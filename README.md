...
- **토큰 소모 최적화**: PPT 문서 파싱 시 과도한 토큰 사용을 방지하기 위해, 이미지 렌더링을 포함하는 기존 스킬(`/md-ppt-render`)과 이미지 파싱을 생략하고 텍스트 기반으로 빠르게 변환하는 신규 스킬(`/md-ppt-text`)로 분리했습니다.
...
- `/md-ppt-text` 스킬에 교차 슬라이드 문맥 방어 규칙(예: '뒷장 계속') 및 플로우차트 기호 해석 로직을 추가하여 파편화를 고도화했습니다.
- `/md-xlsx` 엑셀 정밀 변환 스킬을 신설하여 병합 셀 문맥 복원, 간트 차트 공백 열 압축(세로형 타임라인), 셀 내 줄바꿈(`<br>`) 치환을 자동화했습니다.
- `/md` 계열 변환 스킬의 모든 마크다운 결과물을 무조건 `00. Inbox`로 강제 라우팅하여 볼트 무결성을 지키는 `07_MD_Extraction_Routing_Hook.md`를 도입했습니다.
...
- `/md` : 선택된 파일 형식에 따라 `/md-html`, `/md-ppt-text` 또는 `/md-xlsx`를 자동 호출하는 변환 디스패처
- `/md-html` : 이메일/웹 클리핑 등 HTML 원시 파일 마크다운 정제 및 자동 태깅
- `/md-ppt-render` : PPT 문서 정밀 파싱(이미지 파싱 포함), 보이지 않는 그리드 표 병합, 주석 복원
- `/md-ppt-text` : PPT 텍스트 레이어 기반 빠른 파싱, 이미지 파싱 없이 최대한 다이어그램/표 도식화
- `/md-xlsx` : Excel 시트별 표 및 타임라인 정제 (병합 셀 복원 및 공백 열 압축)
