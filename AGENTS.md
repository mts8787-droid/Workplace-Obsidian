...
- `@include .agent/ki/KI_코어_시스템_지침.md` (하네스 시스템 및 폴더 보호 메타 원칙)
- `@include .agent/ki/KI_Karpathy_Guidelines.md` (안드레 카파시 행동 지침)
- `@include .agent/ki/KI_Harness_Engineering.md` (하네스 진화 및 점진적 학습 관리 원칙)

### 🛠️ SKILLS (에이전트 행동 엔진)
- `@include .agent/skill/01_Directory_Routing_Skill.md` (폴더 자동 라우팅 및 쟈니 데시멀 정밀 분류)
- `@include .agent/skill/02_Metadata_Tagging_Skill.md` (메타데이터 및 태그 자동 매핑)
- `@include .agent/skill/04_Orphan_Linker_Skill.md` (고립된 고아 문서 문맥 연결)
- `@include .agent/skill/05_Daily_Rollup_Skill.md` (일일/주간 작업 요약 리포트 작성)
- `@include .agent/skill/06_Title_Change_Skill.md` (description 기반 파일명 및 타이틀 자동 변경)
- `@include .agent/skill/07_MD_HTML_Convert_Skill.md` (HTML 원시 파일 마크다운 정제 및 자동 태깅)
- `@include .agent/skill/08_MD_PPT_Convert_Skill.md` (PPT/Excel 원시 파일의 정밀 마크다운 변환 및 통합 파싱)
- `@include .agent/skill/09_All_in_One_Classification_Skill.md` (올인원 태깅/분류/라우팅 일괄 처리)

### 🤖 SUBAGENTS (서브에이전트 페르소나 및 롤 정의)
특정 스킬(특히 파싱/정제)을 수행하기 위해 메인 에이전트와 역할을 분담하는 전담 서브에이전트들의 페르소나와 워크플로우 정의입니다.
- `@include .agent/subagent/01_PPT_Parser_Agent.md` (1차 텍스트 및 좌표 원시 추출 파서)
- `@include .agent/subagent/02_PPT_Editor_Agent.md` (파편화된 원시 마크다운의 표/다이어그램 정제 조립)
- `@include .agent/subagent/03_PPT_Reviewer_Agent.md` (조립된 최종 파일의 렌더링 검수 및 무결성 확보)
- `@include .agent/subagent/04_PPT_Writer_Agent.md` (단어 파편의 문장화 및 시각 더미/페이지 번호 삭제 등 최종 구조화)

### 📋 RULES (작업 표준 및 규정)
- `@include .agent/rule/01_Directory_Structure_Rule.md` (폴더 분류 체계)
- `@include .agent/rule/02_Frontmatter_Standard_Rule.md` (15개 필수 속성 및 YAML 문법)
- `@include .agent/rule/03_Tag_Creation_Rule.md` (보수적 태그 생성 원칙)
- `@include .agent/rule/04_Tag_Vocabulary_Rule.md` (허용된 기본 단어장 및 속성 사전)
- `@include .agent/rule/06_Obsidian_Syntax_Rule.md` (옵시디언 특화 렌더링 및 참조 문법 규칙)
- `@include .agent/rule/07_Johnny_Decimal_Rule.md` (쟈니 데시멀 넘버링 정밀 규칙)
- `@include .agent/rule/08_PPT_Parsing_Rule.md` (PPT 타이포그래피, 다이어그램 등 정밀 변환 룰)
- `@include .agent/rule/09_HTML_Parsing_Rule.md` (HTML 태그 제거 및 마크다운 치환 정규화 룰)

### 🚫 HOOKS (절대 금지 사항 - Precedence 0)
- `@include .agent/hook/01_Data_Integrity_Hook.md` (태그 삭제 금지, 문법 파괴 금지, 파편화 금지, .agent 이동 금지)
- `@include .agent/hook/03_PPT_Parsing_Hook.md` (PPT 파싱 시 2개 문서 제한 및 불완전 종료 절대 금지)
